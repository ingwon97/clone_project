...
public class CommentService {
    ...
    public ResponseEntity<?> updateComment(Long postId, Long commentId, CommentRequestDto commentRequestDto) {
        ...
        // CommentRequestDto 의 값을 DB 에 저장함으로써 업데이트
...
//            return new ResponseEntity(ResponseDto.fail("401", "댓글 작성자만 삭제할 수 있습니다."), HttpStatus.UNAUTHORIZED);
    ...
    // 댓글 조회
    public ResponseEntity<?> readComment(Long postId) {

        // 해당 게시글이 존재 하는지 판단
        Optional<Post> post = postRepository.findById(postId);
        if (post.isEmpty()) {
            return new ResponseEntity<>(ResponseDto.fail("404", "해당 게시글이 존재하지 않습니다."), HttpStatus.NOT_FOUND);
        }



        // commentResponstDto 에 하나씩 담아내기
